...
// component can render each variant without prop sprawl. Routed from
// MoreScreen via dedicated screen ids (more-league, more-history, more-admin)
...
export default function MoreSubScreen({ section, state, me, onNav, onSignOut, onReset }) {
  ...
  if (section === 'league') {
    return <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ padding: '14px 20px 20px' }}>
        ...
        <MenuRow
          label="Rules"
          sub={`${ROUNDS_PER_WEEK} drivers/week · Snake`}
          onClick={() => onNav('rules')}
          last
        />
      </div>
  ...
  if (section === 'history') {
    ...
    return <div style={{ paddingBottom: 20 }}>
      <TopBar
        subtitle="Past weeks and last race"
        title="History"
        right={<BackChip onClick={() => onNav('back')}/>}
      />
      ...
      <div style={{ padding: '14px 20px 20px' }}>
        <MenuRow
          label="Draft History"
          sub={isAdmin
            ? `${weeklyResults.length} past week${weeklyResults.length === 1 ? '' : 's'} · tap to view or edit`
            : `${weeklyResults.length} past week${weeklyResults.length === 1 ? '' : 's'}`}
          onClick={() => onNav('history')}
        />
        <MenuRow
          label="Last Race Recap"
          sub={lastResult ? lastResult.track : 'No results yet'}
          onClick={() => onNav('recap')}
          last
        />
      </div>
  ...
  if (section === 'commissioner') {
    return <div style={{ paddingBottom: 20 }}>
      <TopBar
        subtitle="Tools you alone can use"
        title="Commissioner"
        right={<BackChip onClick={() => onNav('back')}/>}
      />
      ...
      <div style={{ padding: '14px 20px 20px' }}>
        <MenuRow
          ...
          sub="Cup one-offs + bonus pools"
          onClick={() => onNav('manage-drivers')}
        />
        <MenuRow
          label="Download Backup"
          sub="Save the full league state as JSON to your device"
          onClick={() => downloadLeagueBackup(state)}
          last
        />
      </div>
      ...
      <div style={{ padding: '14px 20px 40px' }}>
        <MenuRow
          label="Reset Season"
          sub="Erase all results & drafts — type-to-confirm"
      ...
      {resetOpen && <ConfirmResetModal
        ...
        onConfirm={() => { setResetOpen(false); onReset(); onNav('back'); }}
      />}
    </div>;
  }
...
// Trigger a JSON download of the full league state. Used by the commissioner
// to keep a local snapshot outside Supabase as insurance against accidental
...
// Type-to-confirm modal for Reset Season. After 11+ production weeks of
// accumulated state, an accidental reset would be a real catastrophe — so
// we gate the destructive action behind typing the literal word RESET.
// Mirrors the industry-standard pattern (GitHub repo deletion, Stripe).
