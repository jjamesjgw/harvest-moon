...
// Commissioners. Players holding admin powers (entering results, advancing
// the week, resetting drafts, managing drivers). Previously a separate 'admin'
// account existed; that flow was retired so the commissioner doesn't need a
// second login.
//
// This is a LIST so the league isn't a bus factor of one. Week advancement is
// the app's weekly heartbeat and only an admin can do it — if the sole
// commissioner is unreachable on a race weekend, the next slot pick and snake
// draft never open and every other player just sees "Waiting on commissioner."
// Adding a trusted second id here is the cheapest insurance available; it
// needs a deploy, so do it before you need it, not during.
//
// ADMIN_IDS[0] is the PRIMARY commissioner — used wherever a single id is
// required (state.adminId, "who is the commissioner" display). Order matters
// only for that; every entry has identical powers.
export const ADMIN_IDS = ['p_justin', 'p_chad'];

// Back-compat alias for the primary commissioner. Prefer isAdminId() for
// permission checks — a bare `=== ADMIN_ID` comparison silently locks out any
// backup commissioner added above.
export const ADMIN_ID = ADMIN_IDS[0];

// The single permission predicate. Use this everywhere instead of comparing
// ids directly, so adding a backup commissioner is a one-line change here.
export const isAdminId = (id) => !!id && ADMIN_IDS.includes(id);
