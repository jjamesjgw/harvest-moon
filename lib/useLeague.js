...
export function useLeague() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // A realtime push reaching us proves the websocket + Supabase
          // are healthy. If the banner was sticky from an earlier transient
          // failure (boot pull, network blip), clear it now. We DON'T clear
          // when a write is still pending — that write hasn't been confirmed,
          // and the user should still see the banner until it lands.
          if (pendingWriteRef.current == null) {
            setSaveStatus(s => s === 'error' ? 'ok' : s);
            setLastError(null);
          }
        }
      )
  ...
  useEffect(() => {
    ...
    // Wraps pullFresh with bounded exponential backoff. Without this, a
    // transient boot-time fetch failure (offline at startup, slow first
    // network connect, captive portal, etc.) leaves the error banner stuck
    // until the user manually pulls-to-refresh or taps Retry. With this, we
    // self-heal once connectivity arrives — up to 6 attempts capped at 30s.
    const tryPullFresh = async () => {
      try {
        ...
        // Success: clear any banner the boot-error path raised on an earlier
        // attempt. Don't blindly overwrite 'saving'/'idle' — only clear when
        // we're actually in the error state.
        if (!cancelled) {
          setSaveStatus(s => s === 'error' ? 'ok' : s);
          setLastError(null);
        }
        ...
        attempt++;
        if (attempt < 6) {
          const delay = Math.min(30_000, 1500 * Math.pow(2, attempt - 1));
          initRetryTimerRef.current = setTimeout(tryPullFresh, delay);
        }
      }
    };

    tryPullFresh().finally(() => {
      clearTimeout(timer);
  ...
  const flushWrite = useCallback(async (attempt = 1) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // 4xx (excluding 401, handled above) is a permanent client-side
        // rejection: bad-state, bad-json, bad-write-id, bad-client-tag,
        // bad-origin, refused-fresh-over-populated. Retrying the SAME
        // payload won't help — the only path to recovery is the user
        // changing state (next setState replaces pendingWriteRef) or
        // tapping Retry to force a re-attempt. Surface the error and
        // STOP the exp-backoff loop here; otherwise it spends 8 attempts
        // hammering the server with a payload it has already refused.
      ...
      // Only retry transient failures (5xx, network). 4xx is handled above
      // and short-circuits before this catch block.
      if (attempt < 8) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      await pullFresh();
      subscribe();
      // If a write is still queued (a prior save failed and never retried),
      // re-attempt it instead of silently clearing the error banner. Without
      // this, a failed write that's still in pendingWriteRef gets dropped on
      // the next visibility-return refresh — the banner clears but the unsaved
      // diff is lost.
      if (pendingWriteRef.current != null) {
        flushWrite(1);
      } else {
  ...
  }, [refreshing, pullFresh, subscribe, unsubscribe, saveStatus, flushWrite]);
