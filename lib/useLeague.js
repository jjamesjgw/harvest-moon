...
import { BOOT_LOAD_TIMEOUT_MS, SAVE_DEBOUNCE_MS } from './constants';
...
export function useLeague() {
  ...
  useEffect(() => {
    let cancelled = false;
    const timer = setTimeout(() => { if (!cancelled) setLoading(false); }, BOOT_LOAD_TIMEOUT_MS);
  ...
  const setState = useCallback((updater) => {
    setStateLocal((prev) => {
      ...
      // SAVE_DEBOUNCE_MS (800, was 300): typing into Profile fires setState
      // on every keystroke, and at 300ms a fast typer generated one Supabase
      ...
      debounceTimerRef.current = setTimeout(() => flushWrite(1), SAVE_DEBOUNCE_MS);
      return next;
    });
  }, [flushWrite]);
