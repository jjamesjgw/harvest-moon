...
// will be non-empty. Used as a defense-in-depth tripwire in flushWrite to
// refuse the auto-init bug's wipe-write at the database layer, even if the
// gate at the call site is bypassed for any reason.
//
// Note: kept loose enough that legitimate `resetSeason` writes still go
// through, because resetSeason spreads `...s` and therefore preserves any
// player.favDriverNum customizations from Profile edits.
function isFreshShaped(s) {
...
export function useLeague() {
  ...
  return { state, setState, loading, saveStatus, lastError, retry, refresh, refreshing, fetchSucceeded };
}
