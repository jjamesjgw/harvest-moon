...
// Build the Wikipedia article slug for a race. `override`, when provided,
// wins over the name-derived slug — the ingest route passes the schedule
// entry's `wikiSlug` (from lib/data.js) or a runtime
// `state.scheduleOverrides[wk].wikiSlug` escape hatch. This matters for
// disambiguated races: wk 7 (Martinsville) and wk 25 (Richmond) are BOTH
// named "Cook Out 400", so the bare "2026_Cook_Out_400" is a Wikipedia
// disambiguation page, not either race. Both weeks therefore carry an
// explicit wikiSlug in DEFAULT_SCHEDULE.
