...
// `memorial` triggers the in-memoriam treatment (partial desat + diagonal
// black corner band) to mirror primitives.jsx CarNum. See isMemorialDriver
// over there for the matching detection rule.
function drawCarNum(ctx, x, y, size, num, primary, secondary, memorial) {
  const r = size * 0.18;
  ctx.save();
  if (memorial) ctx.filter = 'saturate(55%)';
  // Outer plate
  ...
  ctx.restore();
  // Memorial corner band — drawn outside the desat filter so it's pure
  // black. Clipped to the rounded plate so it follows the corner radius.
  if (memorial) {
    ctx.save();
    roundRect(ctx, x, y, size, size, r);
    ctx.clip();
    ctx.translate(x, y);
    // Match the DOM geometry: horizontal bar centered just inside the
    // top-left corner, rotated -45°. Dims mirror the in-app band exactly.
    const bw = size * 0.7;
    const bh = size * 0.14;
    const cx = bw / 2 - size * 0.25;       // left + width/2 from CarNum
    const cy = bh / 2 + size * 0.03;       // top + height/2 from CarNum
    ctx.translate(cx, cy);
...
// Render + share via the iOS / Android system share sheet. Falls back to
// download on browsers that don't support file sharing (most desktop, older
// Android). On iOS this opens the native share sheet (Messages, AirDrop,
// Photos, group chats) — turns the four-step "download → find file → open
// Messages → attach" flow into a single tap. Returns:
//   'shared'      — user completed share or used save-to-photos via the sheet
//   'cancelled'   — user dismissed the share sheet (don't show success UI)
//   'downloaded'  — we fell back to download (no native share for files)
//   'error'       — render or share failed
//
// Note on detection: navigator.canShare({ files }) is the only reliable test
// for file-share support. Just checking navigator.share returns true on
// browsers that share text but not files.
export function shareOrDownloadCard(payload) {
  return new Promise((resolve) => {
    let canvas;
    try { canvas = renderShareCard(payload); }
    catch (e) { resolve('error'); return; }

    canvas.toBlob(async (blob) => {
      if (!blob) { resolve('error'); return; }
      const filename = `harvest-moon-wk${String(payload.meta.wk).padStart(2, '0')}.png`;
      const file = new File([blob], filename, { type: 'image/png' });

      // Try native share sheet with file payload first.
      if (typeof navigator !== 'undefined'
        ...
        try {
          await navigator.share({
            files: [file],
            title: `Wk ${String(payload.meta.wk).padStart(2, '0')} · Harvest Moon`,
            text: payload.meta.raceName || payload.meta.track,
          });
          resolve('shared');
          return;
        } catch (e) {
          // AbortError means user dismissed the sheet — that's fine, do not
          // fall through to download (they made an active choice).
          if (e && e.name === 'AbortError') { resolve('cancelled'); return; }
          // Other errors (security policy, perm denied) → fall through to download.
        }
      }
