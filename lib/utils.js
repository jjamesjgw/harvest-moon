...
export function detectActiveTurn(state) {
  ...
  // All-Star weeks have no draft and no picker on the clock. Returning
  // null here suppresses the on-the-clock banner and your-turn toast
  // for the duration of the exhibition week. The regular signal
  // resumes automatically once the week advances to a Cup race.
  //
  // Source the schedule from DEFAULT_SCHEDULE rather than state.schedule:
  // /api/notify calls this helper with the RAW Supabase row (no
  // migrateState overlay) and that row's `schedule` is the stale
  // pre-PR schedule where wk 13 = Charlotte with no format flag.
  // Without this, a bogus your-turn push would fire when the week
  // advances into the All-Star exhibition.
  const race = DEFAULT_SCHEDULE.find(s => s.wk === state.currentWeek);
  if (race?.format === 'all-star') return null;
...
// ── FINALIZED-WEEK HELPERS ─────────────────────────────────────────
// EnterResultsScreen's patchWeek writes a weeklyResults row for the CURRENT
// week on every keystroke, but only Save & Advance stamps `finalized: true`.
// So the mere existence of a row does NOT mean the week is complete — while
// the commissioner is mid-entry on race day, an in-progress row is sitting in
// weeklyResults with partial points. Screens that present "completed weeks"
// must filter, or they show a half-entered week as Final.
//
// The predicate is deliberately two-pronged:
//   1. `finalized === true` — the flag written by Save & Advance.
//   2. `wk < currentWeek`   — the week has been advanced past.
// Prong 2 is REQUIRED for correctness, not just belt-and-braces: the
// `finalized` flag was only introduced with the All-Star work (wk 13), so
// every week finalized before that has no flag at all. Filtering on the flag
// alone would silently erase the first ~12 weeks of the season from History,
// Standings, and Recap. Prong 1 in turn covers the final week of a season,
// where currentWeek stops advancing.
export function isWeekFinal(w, currentWeek) {
  if (!w) return false;
  if (w.finalized) return true;
  return typeof currentWeek === 'number' && typeof w.wk === 'number' && w.wk < currentWeek;
}

// Filtered view of weeklyResults containing only completed weeks.
export function finalizedWeeks(weeklyResults, currentWeek) {
  return (weeklyResults || []).filter(w => isWeekFinal(w, currentWeek));
}

// ── SEASON-BOUNDARY HELPERS ────────────────────────────────────────
// Mid-season, finalizing week N advances currentWeek to N+1, so "the results
// that count" is everything through currentWeek - 1 — the current week is by
// definition still in progress.
//
// The final scheduled race breaks that assumption. saveAndAdvance only
// advances when a next week exists (its `hasNext` guard), so after the
// championship is finalized currentWeek stays pinned at the last week. Every
// `currentWeek - 1` reader then permanently excludes the championship race
// from season totals — meaning the app can crown the wrong champion, on the
// one screen everyone looks at, forever.
//
// Rather than advancing currentWeek past the schedule (which would re-arm a
// draft for a week that doesn't exist and ripple through every screen that
// reads currentWeek), these helpers make the boundary depend on whether the
// current week has actually been finalized.

// True once the final scheduled race has been finalized.
export function isSeasonComplete(state) {
  if (!state) return false;
  const { currentWeek, weeklyResults = [] } = state;
  if (typeof currentWeek !== 'number') return false;
  // Source the schedule from code, not state.schedule — the persisted copy
  // lags behind (same reason detectActiveTurn reads DEFAULT_SCHEDULE).
  if (DEFAULT_SCHEDULE.some(sc => sc.wk === currentWeek + 1)) return false;
  return !!weeklyResults.find(w => w.wk === currentWeek)?.finalized;
}

// The last week whose results count toward season standings.
export function standingsThroughWeek(state) {
  if (!state || typeof state.currentWeek !== 'number') return 0;
  return isSeasonComplete(state) ? state.currentWeek : state.currentWeek - 1;
}
...
export function computeStandings(players, weeklyResults, throughWeek) {
  return players.map(p => {
    ...
    mine.forEach(w => {
      ...
      // A weekly win requires actually outscoring the field. Without the
      // `> 0` guard, a week where everyone scored 0 (e.g. the All-Star week
      // when no one picked the race winner: points are 50-or-0) would count
      // as a win for ALL players, since each ties the max of 0.
      const top = vals.length ? Math.max(...vals) : 0;
      if (top > 0 && (w.pts[p.id] || 0) === top) wins++;
    });
...
export function computePlayerDriverStats(playerId, draftHistory, weeklyResults, drivers) {
  ...
  draftHistory.forEach(h => {
    // Skip All-Star exhibition weeks for the same reason as in
    // computeAllDriverStats — synthetic rows with no driverPoints
    // attribution would inflate `picks` and tank `avgPts` for the
    // drivers each player locked in for the bonus week.
    if (h.isAllStar) return;
...
export function computeAllDriverStats(state) {
  ...
  draftHistory.forEach(h => {
    // Skip All-Star exhibition weeks. Their picks are synthetic rows
    // (created in AllStarEntryForm.saveAndAdvance) carrying no
    // driverPoints in weeklyResults, so aggregating them would attribute
    // a phantom zero-pts draft to every All-Star pick and dilute totals,
    // averages, and best/worst-week records.
    if (h.isAllStar) return;
...
// Derive a (label, value) pair describing where the draft currently is.
// Used by OnTheClockBanner and YourTurnToast so non-pickers can see the
// progress without opening the Draft screen. Returns null when the draft
// isn't in an active picking phase.
export function draftProgressLabel(state) {
  const ds = state?.draftState;
  if (!ds) return null;
  if (ds.phase === 'slot-pick') {
    return {
      label: 'Slot pick',
      value: `${(ds.slotPickIdx || 0) + 1} of ${state.players.length}`,
    };
  }
  if (ds.phase === 'snake') {
    const cfg = getWeekConfig(state, state.currentWeek);
    const total = cfg.totalPicks * state.players.length;
    return {
      label: `Round ${ds.currentRound}`,
      value: `Pick ${(ds.picks?.length || 0) + 1} of ${total}`,
    };
  }
  return null;
}
