...
export function computeStandings(players, weeklyResults, throughWeek) {
  return players.map(p => {
    ...
    mine.forEach(w => {
      ...
      // A week nobody scored in has no winner. Without this, an all-zero week
      // credits a "win" to all six players at once — every player's score
      // trivially equals the max of zero. The realistic case is an All-Star
      // week where nobody picked the winner (AllStarEntryForm writes 0 for
      // everyone), which would silently inflate every WinsCount chip in the
      // league. Genuine ties above zero still count for everyone tied.
      if (top <= 0) return;
      if ((w.pts[p.id] || 0) === top) wins++;
    });
